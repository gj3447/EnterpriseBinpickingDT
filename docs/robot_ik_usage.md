...
| 필드 | 설명 |
| --- | --- |
| `pose_targets` | TCP(그리퍼 끝)이 도달해야 할 실제 좌표. 여러 개를 넣으면 첫 번째부터 순서대로 평가합니다. |
...
2. 그리퍼 끝(TCP)과 플랜지 사이의 길이를 **미터 단위**로 `grip_offsets` 배열에 넣습니다.
   - **중요**: `grip_offsets` 는 스칼라(float) 값들을 담는 배열입니다. 예: 그리퍼가 12cm라면 `[0.12]`.
   - `[0, 0, 0.12]` 같은 3차원 벡터를 전달하면 FastAPI 검증에서 422 오류가 발생합니다.
...
1. 디지털 트윈 UI에서는 슬라이더/토글 값이 바뀌면 `performIkAutoSolve()` 가 자동 호출되고,  
   `말단 Z축 아래 고정` 토글 상태에 따라 `/api/robot/ik/ikpy` 또는 `/api/robot/ik/ikpy/downward` 프록시로 요청을 전송합니다.
